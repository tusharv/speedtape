...
const BAR_PAD = "31415926";
const BAR_COUNT = 36;

type Inspect = (term: TermKey) => void;
...
function PassBars({ test }: { test: SpeedTestRow }) {
  const bars = useMemo(() => {
    ...
    return seed.replace(/\D/g, "").padEnd(28, BAR_PAD).slice(0, BAR_COUNT).split("");
  ...
  return (
    <div className="grid grid-cols-1 gap-3 border-b border-hairline py-4 sm:grid-cols-3">
      ...
      <PassField
        term="outage"
        label="Outage"
        value={formatOutageDuration(outage.wentDownAt, outage.restoredAt)}
  ...
  return (
    <div className="grid grid-cols-2 gap-3 py-4 sm:grid-cols-5">
      ...
      <PassField
        ...
        label="Up"
        value={
          <SpeedReading value={test.uploadMbps} format={formatMbps} unit="Mbps" />
        }
      ...
      <PassField
        term="ping"
        label="Ping"
        value={<SpeedReading value={test.pingMs} format={formatMs} unit="ms" />}
        active={active === "ping"}
        onInspect={onInspect}
      />
      <PassField
        term="jitter"
        label="Jitter"
        value={<SpeedReading value={test.jitterMs} format={formatMs} unit="ms" />}
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        <div className="relative flex min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <PassHeader failed={failed} runLabel={runLabel} serial={serial} />
            ...
            <div className="relative px-4 py-5 sm:px-5">
              ...
              <PassRoute test={test} active={active} onInspect={setActive} />
              <PassTicketMeta
                test={test}
                when={when}
                failed={failed}
              ...
              <PassMetrics test={test} active={active} onInspect={setActive} />
              ...
              <PassFooter test={test} neighbors={neighbors} serial={serial} />
            </div>
          </div>
        </div>
